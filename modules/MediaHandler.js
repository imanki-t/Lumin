...
export async function processPromptAndMediaAttachments(prompt, message, attachments = null) {
  ...
  for (const part of processed) {
    ...
    if (Array.isArray(part)) {
      for (const p of part) {
        // Push ALL parts from the array — both the text metadata label AND
        // the fileData part. The old guard (fileUri || fileData || inlineData)
        // was silently dropping the text label, which caused photos/gifs to
        // lose their context description in the Gemini request.
        if (p && (p.text !== undefined || p.fileUri || p.fileData || p.inlineData)) {
          parts.push(p);
        }
      }
    } else if (part.text !== undefined || part.fileUri || part.fileData || part.inlineData) {
      parts.push(part);
    }
  }
